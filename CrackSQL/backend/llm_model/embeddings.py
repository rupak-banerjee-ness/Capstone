...
from litellm import embedding as litellm_embedding, aembedding as litellm_aembedding
...
class LiteLLMEmbeddings:
    """LangChain-style embedding adapter backed by LiteLLM (OpenAI, Bedrock, Gemini, ...)"""
    ...
    def embed_query(self, text: str) -> List[float]:
        response = litellm_embedding(model=self.model_name, input=[text], **self._kwargs())
        return response.data[0]['embedding']

    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        response = litellm_embedding(model=self.model_name, input=texts, **self._kwargs())
        return [item['embedding'] for item in response.data]

    async def aembed_query(self, text: str) -> List[float]:
        response = await litellm_aembedding(model=self.model_name, input=[text], **self._kwargs())
        return response.data[0]['embedding']

    async def aembed_documents(self, texts: List[str]) -> List[List[float]]:
        response = await litellm_aembedding(model=self.model_name, input=texts, **self._kwargs())
        return [item['embedding'] for item in response.data]
    ...
    def release_embedding(self, model_name: str):
        ...
        if model_name in self._embeddings:
            ...
            try:
                ...
                # LiteLLM-backed models don't need special release operations
    ...
    def load_embedding(self, model_config: dict):
        ...
        try:
            if model_config['deployment_type'] == 'cloud':
                # Cloud model, routed through LiteLLM (OpenAI, Bedrock, Gemini, Vertex AI, ...)
                embedding = LiteLLMEmbeddings(
                    model_name=model_config['name'],
                    api_key=model_config.get('api_key'),
                    api_base=model_config.get('api_base'),
                    aws_region_name=model_config.get('aws_region_name')
                )
            else:
                import torch
                # Determine device
